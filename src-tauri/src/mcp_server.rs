use crate::mcp_buffer::McpBuffer;
...
impl Clone for AppState {
    fn clone(&self) -> Self {
        Self {
            ...
            serial: self.serial.inner_clone(),
        }
    }
}
    ...
    match method {
        ...
        "resources/read" => {
            ...
            match uri {
                ...
                "serial://port" => {
                    let name = state.serial.port_name.lock().await.clone().unwrap_or_default();
                    let connected = state.serial.connected.load(std::sync::atomic::Ordering::SeqCst);
                    let tx = state.serial.tx_bytes.load(std::sync::atomic::Ordering::SeqCst);
                    let rx = state.serial.rx_bytes.load(std::sync::atomic::Ordering::SeqCst);
                    let baud = state.serial.baud_rate.load(std::sync::atomic::Ordering::SeqCst);
